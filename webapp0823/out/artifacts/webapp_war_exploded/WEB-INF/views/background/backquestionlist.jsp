...
<div class="container-fluid" style="padding-top: 10%">
            ...
            <table class="listborder table table-striped table-hover">
                ...
                <c:forEach items='${questionList}' var="question">
                    <tr class="listbody">
                        ...
                        <td><button name="btn-delete" value="${question.ID}" type="button" class="btn btn-success">删除</button></td>
                    </tr>
                </c:forEach>
                </tbody>
            </table>
    ...
    <script>
        $('#search-form').on('submit',1,refreshQuestionList);
        //绑定事件
        $('button[name="btn-delete"]').on('click',deleteQuestion);
        ...
        function deleteQuestion(event){
            var questionid = $(event.target).val();
            if(confirm('您确定要删除吗？')){
                $.ajax({
                    url:'/platforminfo/admin/deletequestion/'+questionid,
                    cache:false,
                    success:function (data) {
                        if('success' == data){
                            alert("删除成功！");
                            loadPage('/platforminfo/admin/questionlist')
                        }else{
                            alert("删除失败！请重试。");
                        }
                    }
         ...
         * @returns {boolean}
         */
        function refreshQuestionList(event){
            var num;
            ...
            $.ajax({
                ...
                url:'/platforminfo/searchquestion/'+num,
                cache:false,
                dataType:'json',
                success:function (data) {
                    //修改分页器属性
                    ...
                    $('#tbody').empty();
                    //相tbody填充新内容
                    $.each(data.questionlist,function (idx,question) {
                        $('#tbody').append("<tr class=\"listbody\">\n" +
                            "                            <td>"+question.id+"</td>\n" +
                            "                            <td>"+question.title+"</td>\n" +
                            "                            <td>"+question.author+"</td>\n" +
                            "                            <td>"+question.datetime+"</td>\n" +
                            "                            <td><button id=\"btn-delete\" value=\""+question.id+"\" type=\"button\" class=\"btn btn-success\">删除</button></td>\n" +
                            "                        </tr>");
                    });
                    //绑定事件
                    $('button[name="btn-delete"]').on('click',deleteQuestion);
